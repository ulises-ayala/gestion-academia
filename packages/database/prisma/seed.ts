...
const seedPassword = process.env.DEV_SEED_PASSWORD ?? 'AcademiaDev2026!';
...
const assertDevelopmentDatabase = () => {
  if (process.env.NODE_ENV !== 'development') {
    throw new Error('Seed cancelado: NODE_ENV debe ser exactamente "development".');
  }
  if (process.env.CI) {
    throw new Error('Seed cancelado: no se permite ejecutar el seed de desarrollo en CI.');
  }

  const rawUrl = process.env.DATABASE_URL;
  if (!rawUrl) throw new Error('Seed cancelado: falta DATABASE_URL.');
  ...
  if (
    ...
    throw new Error(
      'Seed cancelado: sólo se permite PostgreSQL local (localhost/127.0.0.1/::1), base "academy".',
    );
  }
};
  ...
  const summary = await prisma.$transaction(
    async (tx) => {
      const adminDefinitions = [
        ['admision', AdminRole.RECEPTION],
        ['administracion', AdminRole.MANAGER],
        ['direccion', AdminRole.ADMINISTRATOR],
      ] as const;
      ...
      const scheduleDefinitions = [
        ...
        [DayOfWeek.TUESDAY, 18, 19, 2],
        [DayOfWeek.WEDNESDAY, 18, 19, 3],
