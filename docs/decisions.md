...
## Ajustes de facturación v1

131. **Condición recurrente y snapshot mensual:** becas y descuentos se configuran por inscripción con vigencia mensual. Al generar o afectar una cuota se crea un `MonthlyChargeAdjustment` append-only; cambiar o finalizar una condición nunca reescribe snapshots históricos.
132. **Dos bases explícitas:** la deuda del alumno y la futura base de liquidación se derivan por separado desde el importe base. La beca de Dirección reduce solo la deuda; becas y descuentos docentes reducen ambas. Cada porcentaje se calcula independientemente sobre el importe base y ambos resultados se limitan a cero.
