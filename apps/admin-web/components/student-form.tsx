...
export function StudentForm({
  ...
  onSaved,
  onCancel,
}: Readonly<{ student?: StudentDto; onSaved(student: StudentDto): void; onCancel?(): void }>) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const saved = await apiRequest<StudentDto>(
        student ? `/students/${student.id}` : '/students',
        { method: student ? 'PATCH' : 'POST', body: JSON.stringify(payload) },
      );
      onSaved(saved);
    } catch (error) {
  ...
  const input = (field: keyof FormState, value: string) => {
  ...
  const error = (field: keyof FormState) =>
    ...
    <form className="student-form" onSubmit={submit} noValidate>
      <label>
        ...
        <input
          ...
          value={form.dni}
          onChange={(event) => input('dni', event.target.value)}
        />
        {error('dni')}
      </label>
      <label>
        ...
        <input
          ...
          onChange={(event) => input('firstName', event.target.value)}
        />
        {error('firstName')}
      </label>
      <label>
        ...
        <input
          ...
          value={form.lastName}
          onChange={(event) => input('lastName', event.target.value)}
        />
        {error('lastName')}
      </label>
      <label>
        ...
        <input
          ...
          onChange={(event) => input('birthDate', event.target.value)}
        />
        {error('birthDate')}
      </label>
      <label>
        ...
        <input
          ...
          value={form.phone}
          onChange={(event) => input('phone', event.target.value)}
        />
        {error('phone')}
      </label>
      <label>
        ...
        <input
          ...
          value={form.email}
          onChange={(event) => input('email', event.target.value)}
        />
        {error('email')}
      </label>
      ...
      <div className="actions wide">
        <button disabled={submitting} type="submit">
          {submitting ? 'Guardando…' : student ? 'Guardar cambios' : 'Crear alumno'}
        </button>
        {onCancel && (
