...
export function AuthProvider({ children }: Readonly<{ children: ReactNode }>) {
  ...
  useEffect(() => {
    void (async () => {
      try { setUser((await apiRequest<{ user: AuthUserDto }>('/auth/me')).user); }
      catch {
        try { setSetupRequired((await apiRequest<SetupStatusDto>('/auth/setup-status')).required); } catch { /* API no disponible */ }
  ...
  if (!ready) return <main className="auth-shell"><p>Comprobando sesión…</p></main>;
  if (!user) return <LoginScreen setupRequired={setupRequired} onAuthenticated={setUser} />;
  return <AuthContext.Provider value={{ user, logout }}>{children}</AuthContext.Provider>;
}

function LoginScreen({ setupRequired, onAuthenticated }: Readonly<{ setupRequired: boolean; onAuthenticated(user: AuthUserDto): void }>) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const session = await apiRequest<AuthSessionDto>(`/auth/${setupRequired ? 'bootstrap' : 'login'}`, { method: 'POST', body: JSON.stringify(credentials) });
      onAuthenticated(session.user);
    } catch (error) { setMessage(error instanceof ApiClientError ? error.message : 'No se pudo conectar con la API'); }
  ...
  return <main className="auth-shell"><section className="card auth-card">
    ...
    <form className="auth-form" onSubmit={submit}>
      ...
      <button disabled={submitting} type="submit">{submitting ? 'Procesando…' : setupRequired ? 'Crear administrador' : 'Ingresar'}</button>
    </form>
