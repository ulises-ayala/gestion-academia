...
const mapClass = (item: IncludedClass): ClassData => ({
  id: item.id,
  name: item.name,
  level: item.level,
  ...
  schedules: item.schedules.map((schedule) => ({
    ...
    startTime: fromTime(schedule.startTime),
    endTime: fromTime(schedule.endTime),
    room: schedule.room,
  })),
  activeEnrollmentCount: item._count.enrollments,
...
export class PrismaClassRepository implements ClassRepository {
  ...
  async findById(id: string) {
    const item = await this.prisma.academyClass.findUnique({ where: { id }, include });
    return item ? mapClass(item) : null;
  }
  findTeacher(id: string) {
    return this.prisma.teacher.findUnique({ where: { id }, select: { status: true } });
  }
  findDanceType(id: string) {
    return this.prisma.danceType.findUnique({ where: { id }, select: { status: true } });
  }
  async findRooms(ids: readonly string[]) {
    const rooms = await this.prisma.room.findMany({
      where: { id: { in: [...ids] } },
      select: { id: true, status: true, branch: { select: { status: true } } },
    });
    return rooms.map((room) => ({
      id: room.id,
      status: room.status,
      branchStatus: room.branch.status,
    }));
  }
  ...
  async findPage(query: ClassQuery) {
    const where: Prisma.AcademyClassWhereInput = {
      ...
      ...(query.danceTypeId ? { danceTypeId: query.danceTypeId } : {}),
      ...(query.teacherId ? { teacherId: query.teacherId } : {}),
      ...(query.branchId
        ? { schedules: { some: { status: 'ACTIVE', room: { branchId: query.branchId } } } }
        : {}),
    };
    const [items, total] = await this.prisma.$transaction([
      ...
      this.prisma.academyClass.count({ where }),
    ]);
  ...
  private async write(id: string | undefined, data: ValidClassInput): Promise<ClassData> {
    return this.prisma.$transaction(
      async (tx) => {
        ...
        let classId = id;
        if (id) {
          await tx.academyClass.update({ where: { id }, data: classData });
          await tx.classSchedule.updateMany({
            where: { classId: id, status: 'ACTIVE' },
            data: { status: 'INACTIVE' },
          });
        } else {
          classId = (await tx.academyClass.create({ data: classData })).id;
        }
        ...
        await tx.classSchedule.createMany({
          data: data.schedules.map((schedule) => ({
            classId,
            dayOfWeek: schedule.dayOfWeek,
            startTime: atTime(schedule.startTime),
            endTime: atTime(schedule.endTime),
  ...
  private async assertNoConflicts(
    ...
    const intervalConditions = data.schedules.map((schedule) => ({
      dayOfWeek: schedule.dayOfWeek,
      startTime: { lt: atTime(schedule.endTime) },
      endTime: { gt: atTime(schedule.startTime) },
    }));
    const base = {
      status: 'ACTIVE' as const,
      class: { status: 'ACTIVE' as const, ...(classId ? { id: { not: classId } } : {}) },
    };
    const roomConflict = await tx.classSchedule.findFirst({
      where: {
        ...base,
        OR: data.schedules.map((schedule) => ({
          roomId: schedule.roomId,
          dayOfWeek: schedule.dayOfWeek,
          startTime: { lt: atTime(schedule.endTime) },
          endTime: { gt: atTime(schedule.startTime) },
    ...
    if (roomConflict)
      throw new DomainError(
        ...
        'El salón ya está ocupado en parte de ese horario',
        { field: 'schedules' },
      );
    const teacherConflict = await tx.classSchedule.findFirst({
      where: {
        ...base,
        class: { ...base.class, teacherId: data.teacherId },
    ...
    if (teacherConflict)
      throw new DomainError(
        ...
        'El profesor ya tiene otra clase en parte de ese horario',
        { field: 'schedules' },
      );
  }
}
