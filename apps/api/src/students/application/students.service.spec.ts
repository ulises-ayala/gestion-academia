...
describe('StudentsService', () => {
  ...
  it('filtra activos e inactivos', async () => {
    ...
    await expect(service.list({ status: 'ACTIVE', page: 1, pageSize: 25 })).resolves.toMatchObject({
      total: 1,
      items: [{ firstName: 'Juan' }],
    });
    await expect(
      service.list({ status: 'INACTIVE', page: 1, pageSize: 25 }),
    ).resolves.toMatchObject({ total: 1, items: [{ firstName: 'Ana' }] });
  });
  ...
  it('pagina sin cargar todos los resultados', async () => {
    ...
    await expect(service.list({ page: 2, pageSize: 2 })).resolves.toMatchObject({
      total: 5,
      page: 2,
      pageSize: 2,
      items: [{ firstName: 'Alumno 2' }, { firstName: 'Alumno 3' }],
    });
  });

  it('informa alumno inexistente', async () => {
    await expect(service.get(crypto.randomUUID())).rejects.toMatchObject({
  ...
  it('impide desactivar si el alumno tiene inscripciones activas', async () => {
    const created = await service.create({ dni: '12345678', firstName: 'Ana', lastName: 'Pérez' });
    const guarded = new StudentsService(repository, {
      assertStudentCanDeactivate: async () => {
        throw new DomainError('STUDENT_HAS_ACTIVE_ENROLLMENTS', 'Tiene inscripciones activas');
      },
    } as never);
