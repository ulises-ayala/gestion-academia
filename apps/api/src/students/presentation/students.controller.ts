...
export class StudentsController {
  ...
  @Get()
  list(@Query('q') q?: string, @Query('status') status?: string, @Query('page') pageValue?: string, @Query('pageSize') pageSizeValue?: string) {
    ...
    if (q !== undefined && (typeof q !== 'string' || q.trim().length > 100)) {
      throw new DomainError('VALIDATION_ERROR', 'La búsqueda no puede superar los 100 caracteres', { field: 'q' });
    }
  ...
  private parsePositiveInteger(value: string | undefined, field: string, fallback: number, maximum: number): number {
    if (value === undefined || value === '') return fallback;
    if (!/^\d+$/.test(value)) throw new DomainError('VALIDATION_ERROR', `${field} debe ser un entero positivo`, { field });
    const parsed = Number.parseInt(value, 10);
    if (parsed < 1 || parsed > maximum) throw new DomainError('VALIDATION_ERROR', `${field} debe estar entre 1 y ${maximum}`, { field });
    return parsed;
  }
}
