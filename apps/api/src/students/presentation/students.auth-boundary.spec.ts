...
import {
  ...
  type AuthRepository,
  type AuthUser,
} from '../../auth/application/auth.repository';
...
class BoundaryAuthRepository implements AuthRepository {
  ...
  async findUserBySession(tokenHash: string, now: Date) {
    ...
    return session && session.expiresAt > now
      ? (this.users.find((user) => user.id === session.userId) ?? null)
      : null;
  }
...
describe('Students authentication boundary', () => {
  ...
  it('permite acceder a un usuario administrativo autenticado', async () => {
    const bootstrap = await fetch(`${rootUrl}/auth/bootstrap`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username: 'admin', password: 'una-clave-segura' }),
    });
