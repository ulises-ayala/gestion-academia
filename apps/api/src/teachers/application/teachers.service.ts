...
export class TeachersService {
  ...
  list(query: TeacherQuery) {
    return this.repository.findPage(query);
  }
  async get(id: string) {
    const teacher = await this.repository.findById(id);
    if (!teacher) throw new DomainError('TEACHER_NOT_FOUND', 'Profesor no encontrado');
    return teacher;
  }
  async create(input: TeacherInput) {
    const data = validateTeacher(input);
  ...
  async update(id: string, patch: Partial<TeacherInput>) {
    ...
    if (
      ...
      (await this.repository.hasActiveClasses(id))
    )
      throw new DomainError(
        'TEACHER_IN_USE',
        'No se puede desactivar un profesor asignado a clases activas',
      );
    const data = validateTeacher({
