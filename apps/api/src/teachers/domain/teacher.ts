...
export type TeacherData = Readonly<{
  id: string;
  dni: string;
  firstName: string;
  lastName: string;
...
export const validateTeacher = (input: TeacherInput) => {
  ...
  return {
    dni: normalizeTeacherDni(input.dni),
    firstName: text(input.firstName, 'firstName', 100),
    lastName: text(input.lastName, 'lastName', 100),
    phone: optional(input.phone, 'phone', 50),
    email,
    address: optional(input.address, 'address', 500),
