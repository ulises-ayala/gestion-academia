...
const publicUser = ({ passwordHash: _passwordHash, ...user }: Awaited<ReturnType<AuthRepository['findUserByUsername']>> & {}) => user;

@Injectable()
export class AuthService {
  constructor(@Inject(AUTH_REPOSITORY) private readonly repository: AuthRepository) {}

  async setupRequired(): Promise<boolean> { return (await this.repository.countUsers()) === 0; }

  async bootstrap(usernameValue: unknown, passwordValue: unknown): Promise<AuthResult> {
    if (!(await this.setupRequired())) throw new DomainError('AUTH_ALREADY_CONFIGURED', 'La configuración inicial ya fue realizada');
  ...
  async authenticate(token: string | undefined): Promise<PublicAuthUser> {
    ...
    if (!user || user.status !== 'ACTIVE') throw new DomainError('UNAUTHORIZED', 'La sesión no es válida o venció');
  ...
  private async startSession(user: NonNullable<Awaited<ReturnType<AuthRepository['findUserByUsername']>>>): Promise<AuthResult> {
    const token = randomBytes(32).toString('base64url');
    const hours = Number.parseInt(process.env.AUTH_SESSION_HOURS ?? '12', 10);
    const expiresAt = new Date(Date.now() + (Number.isFinite(hours) && hours > 0 ? hours : 12) * 60 * 60 * 1000);
  ...
  private normalizeUsername(value: unknown): string {
    if (typeof value !== 'string') throw new DomainError('VALIDATION_ERROR', 'El usuario es obligatorio', { field: 'username' });
    const username = value.trim().toLowerCase();
    if (!/^[a-z0-9._-]{3,100}$/.test(username)) throw new DomainError('VALIDATION_ERROR', 'El usuario debe tener entre 3 y 100 caracteres y usar letras, números, punto, guion o guion bajo', { field: 'username' });
    return username;
  }
}
