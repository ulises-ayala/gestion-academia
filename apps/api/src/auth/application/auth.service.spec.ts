...
class MemoryAuthRepository implements AuthRepository {
  ...
  async findUserByUsername(username: string) { return this.users.find((item) => item.username === username) ?? null; }
  ...
  async findUserBySession(tokenHash: string, now: Date) { const session = this.sessions.get(tokenHash); return session && session.expiresAt > now ? this.users.find((item) => item.id === session.userId) ?? null : null; }
...
describe('AuthService', () => {
  ...
  beforeEach(() => { repository = new MemoryAuthRepository(); auth = new AuthService(repository); });
  ...
  it('crea una sola cuenta administrativa inicial e inicia sesión', async () => {
    ...
    await expect(auth.bootstrap('otro', 'otra-clave-segura')).rejects.toMatchObject({ code: 'AUTH_ALREADY_CONFIGURED' });
  });
  ...
  it('rechaza credenciales incorrectas e invalida la sesión al salir', async () => {
    ...
    await expect(auth.login('admin', 'clave-incorrecta')).rejects.toMatchObject({ code: 'INVALID_CREDENTIALS' });
