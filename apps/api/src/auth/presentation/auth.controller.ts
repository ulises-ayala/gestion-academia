...
type HttpResponse = { cookie(name: string, value: string, options: Record<string, unknown>): void; clearCookie(name: string, options: Record<string, unknown>): void };
type HttpRequest = { headers: Record<string, string | string[] | undefined>; adminUser?: unknown };
const cookieOptions = (expires?: Date) => ({ httpOnly: true, sameSite: 'lax' as const, secure: process.env.NODE_ENV === 'production', path: '/', ...(expires ? { expires } : {}) });
...
export class AuthController {
  ...
  @Public()
  @Get('setup-status')
  async setupStatus() { return { required: await this.auth.setupRequired() }; }
  ...
  @Get('me')
  me(@Req() request: HttpRequest) { return { user: request.adminUser }; }

  @HttpCode(204)
  @Post('logout')
  async logout(@Req() request: HttpRequest, @Res({ passthrough: true }) response: HttpResponse): Promise<void> {
